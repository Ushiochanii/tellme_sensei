...
path = Path("app/ui/settings_window.py")
...
def replace_once(old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(
            f"expected one SettingsWindow patch anchor, found {count}: {old[:80]!r}"
        )
...
workers = '''class UpdateCheckWorker(QObject):
...
'''
replace_once("class SettingsWindow(QWidget):\n", workers + "class SettingsWindow(QWidget):\n")
...
updates_page = '''        updates_page, updates_layout = make_page(
...
replace_once(
    ...
    updates_page + "        debug_page, debug_layout = make_page(\n",
)
replace_once(
    "            auto_watch_page,\n"
    "            debug_page,\n",
